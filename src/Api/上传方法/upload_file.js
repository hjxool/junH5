...
(function () {
	...
	function slice_upload(index, slice_list, md5, file, obj, fail_index) {
		...
		r.onload = async (e) => {
			...
			// }).catch(err => {
			// 	// 配置 上传失败时执行
			// 	obj.uploadFail(err, file);
			// });

			let data = await 请求接口(obj.upload_url, 'post', form_obj, {'Oss-Bucket-Name': obj?.cus_header || 'file-center-storage'});
			if(!data) {
